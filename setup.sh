...
git clone --depth 1 --branch "$BRANCH" "$REPO_URL" "$INSTALL_DIR" || { echo "Git clone failed"; exit 1; }
...
for app in "${apps[@]}"; do
  ...
  if [[ -d "$SRC" ]]; then
    echo " → rsync $SRC → $DEST"
    rsync -a --delete "$SRC" "$DEST" || { echo "Rsync failed for $SRC"; exit 1; }
  else
    echo "Warning: '$SRC' missing; skipping."
  fi
done

# ASCII art using cat and a here document
cat << "EOF"
____       _                                   
